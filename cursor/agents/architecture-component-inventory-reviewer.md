...
description: Fresh architecture component inventory reviewer. Use to verify `.cursor/state/architecture-documentation-state.md` against repo reality and add missing architecture-documentation components when evidence supports them.
model: inherit
---
...
- `.cursor/state/architecture-inventory-review-state.md`
...
If the review state is missing, create it from `.cursor/state/architecture-inventory-review-state.example.md`.
...
active_findings:
  - id: "INV-R1-P1"
    ...
    evidence:
      ...
      docs:
        - "AGENTS.md identifies job lifecycle as a high-signal entrypoint."
...
- Round: <n>/20
- State updated: `.cursor/state/architecture-inventory-review-state.md`
- Inventory state: `.cursor/state/architecture-documentation-state.md`

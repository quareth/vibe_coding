...
Prefer `.cursor/skills/feature-implementation-workflow/SKILL.md` for normal
...
- `.cursor/state/implementation-state.md` — profile, guide, exact task,
  persisted next-task handoff, and lifecycle status.
- `.cursor/state/implementation-review-state.md` — current functional
  reviewer/fixer cycle.
- `.cursor/state/implementation-quality-review-state.md` — Medium/High frozen
  Git quality scope.
- `.cursor/state/implementation-guide-state.md` and
  `.cursor/state/implementation-guide-review-state.md` — Medium/High
...
1. Require an implementation guide.
   ...
   - If the user has only a request or reference documents, call
     `/implementation-guide-creator` first.
...
2. Call `/feature-implementer` for exactly one task.
3. Route implementation-state:
   - `TASK_COMPLETE` -> call `/feature-implementer next`.

...
Template for `.cursor/state/architecture-documentation-state.md`.

This is the durable source of truth for architecture documentation inventory, per-component progress, and daily drift-audit metadata. Copy the YAML block into `.cursor/state/architecture-documentation-state.md` when starting or resetting the workflow.
...
components:
  - id: "api-boundary"
    ...
    related_components:
      ...
      - "persistence"
    evidence_notes:
      - "AGENTS.md identifies src/app.py and the API router as wired entrypoints."
    last_doc_updated_at: ""

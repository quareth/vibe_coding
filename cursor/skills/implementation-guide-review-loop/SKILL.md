...
description: Run a state-driven review/fix loop for implementation-guide documents only (not code), using `.cursor/state/implementation-guide-state.md` and `.cursor/state/implementation-guide-review-state.md`. Use when the user asks to review an implementation guide, run blocker-only guide review, or loop reviewer/fixer until guide blockers are resolved.
---
...
Durable files:
- `.cursor/state/implementation-guide-state.md` - guide path and review scope.
- `.cursor/state/implementation-guide-review-state.md` - current-cycle blocker ledger.
...
- Use `phase` from `.cursor/state/implementation-guide-state.md`.
- Verify the phase against the full guide, related design, and current repo reality where needed for contradiction checks.

## Loop behavior

1. Read `.cursor/state/implementation-guide-state.md`.
2. Initialize/reset `.cursor/state/implementation-guide-review-state.md` with clean `active_findings: []`.

...
description: Run a repository-local state-driven architecture documentation workflow through `.claude/state/architecture-documentation-state.md`. Use when the user asks to discover repository architecture components, create or continue architecture docs, document components, review generated architecture docs, or continue architecture documentation from state.
---
...
Durable files:
- `.claude/state/architecture-documentation-state.md` - component inventory, current component, doc progress, and drift metadata.
- `.claude/state/architecture-inventory-review-state.md` - current-cycle inventory review ledger.
- `.claude/state/architecture-doc-review-state.md` - current-cycle component doc review ledger.

## Workflow

1. Read `.claude/state/architecture-documentation-state.md`.
2. If missing, initialize from `.claude/state/architecture-documentation-state.example.md`.
3. Route by state:
   - `DISCOVERY_READY`: call `architecture-component-discoverer`.
   - `INVENTORY_REVIEW_READY`: reset `.claude/state/architecture-inventory-review-state.md` to clean `READY_FOR_REVIEW`, then call a fresh `architecture-component-inventory-reviewer`.
   ...
   - `DOC_REVIEW_READY`: reset `.claude/state/architecture-doc-review-state.md` for `mode: component_doc`, then call a fresh `architecture-doc-reviewer`.
...
- Code is the source of truth; verify claims through wired paths from `CLAUDE.md`.

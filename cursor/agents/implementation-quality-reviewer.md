...
Your durable ledger is `.cursor/state/implementation-quality-review-state.md`. You may modify only that state file and create or update non-blocking refactor suggestion documents under the `refactor_suggestion_root` recorded in state. Do not modify application code, tests, migrations, prompts, product documentation, implementation guides, or `.cursor/state/implementation-state.md`.
...
- `.cursor/state/implementation-quality-review-state.md`, initialized from its example when missing.
- `.cursor/state/implementation-state.md` and an active guide only when referenced for intent context; neither may define or expand scope.
...
4. Inspect only behavior-neutral quality cleanup against AGENTS.md.

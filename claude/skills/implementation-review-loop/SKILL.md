...
description: Run a repository-local state-driven implementation review loop through `.claude/state/implementation-review-state.md`. Use when the user asks to review current implementation work, review a phase or task, run final implementation review, run review-and-fix until no blockers, or mentions implementation-review-state, blocker ledger, reviewer/fixer loop, or fresh full review.
---
...
- `.claude/state/implementation-state.md` - guide, intent, ownership checklist, and optional current phase/task for feature-implementer workflow.
- `.claude/state/implementation-review-state.md` - current-cycle blocker ledger and neutral scope metadata.
...
- Before calling the next reviewer after a fix, ensure `.claude/state/implementation-review-state.md` has no previous findings, no fix attempts, and no review history.
...
Scope:
- Review only the current `guide`, `phase`, and `task` from `.claude/state/implementation-state.md`.
...
Scope:
- Review all tasks and phase acceptance criteria for the current `phase` from `.claude/state/implementation-state.md`.
...
Scope:
- Review the full guide referenced by `.claude/state/implementation-state.md`, unless the user names a different guide or scope.
- Ignore `phase` and `task` in `.claude/state/implementation-state.md`; those are only for feature-implementer/current-task review.
...
1. Read `.claude/state/implementation-state.md`.
2. Initialize or reset `.claude/state/implementation-review-state.md` for the selected mode with clean `active_findings: []`.

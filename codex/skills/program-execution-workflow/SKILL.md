...
description: Pure handoff router for repository-local programs from `.codex/agents/program-workflow-state.md`. Discovers all numbered phase files under doc_root and runs the full create→guide-review→implement→final-implementation-review→quality-review cycle for every remaining item in one session until program_complete or hard_stop. Does not stop after one phase. Does not create, review, or implement itself.
---

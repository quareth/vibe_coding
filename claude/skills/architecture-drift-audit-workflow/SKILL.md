...
Durable files:
- `.claude/state/architecture-documentation-state.md` - component inventory and drift metadata.
- `.claude/state/architecture-doc-review-state.md` - current-cycle drift findings and review ledger.
...
3. For each component with `status: doc_complete` and an existing `doc_path`:
   ...
   - reset `.claude/state/architecture-doc-review-state.md` with `mode: drift_audit`, clean `active_findings: []`, component id, and doc path,

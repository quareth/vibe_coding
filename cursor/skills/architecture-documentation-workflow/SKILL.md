...
Durable files:
- `.cursor/state/architecture-documentation-state.md` - component inventory, current component, doc progress, and drift metadata.
- `.cursor/state/architecture-inventory-review-state.md` - current-cycle inventory review ledger.
- `.cursor/state/architecture-doc-review-state.md` - current-cycle component doc review ledger.

## Workflow

1. Read `.cursor/state/architecture-documentation-state.md`.
2. If missing, initialize from `.cursor/state/architecture-documentation-state.example.md`.
3. Route by state:
   - `DISCOVERY_READY`: call `architecture-component-discoverer`.
   - `INVENTORY_REVIEW_READY`: reset `.cursor/state/architecture-inventory-review-state.md` to clean `READY_FOR_REVIEW`, then call a fresh `architecture-component-inventory-reviewer`.
   ...
   - `DOC_REVIEW_READY`: reset `.cursor/state/architecture-doc-review-state.md` for `mode: component_doc`, then call a fresh `architecture-doc-reviewer`.

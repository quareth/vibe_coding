...
Template for `.cursor/state/implementation-quality-review-state.md`. The live state file is ignored by Git. Copy the clean state, set one branch or commit target, and let the first reviewer freeze the exact Git diff.
...
round: 0
implementation_state: ".cursor/state/implementation-state.md" # optional intent context only
